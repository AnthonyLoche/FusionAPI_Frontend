<template>
  <v-navigation-drawer v-model="drawer" class="sidebar" :permanent="!isMobile" :rail="rail" rail-width="85"
    ...
    <div class="nav-section">
      ...
      <v-list class="nav-list" density="compact" nav>
        <v-list-item v-for="item in cadastroItems" :key="item.route" :active="isActive(item.route)" class="nav-item"
          rounded="lg" :to="item.route">
          <template #prepend>
            <v-icon :class="{ 'icon-active': isActive(item.route) }" :icon="item.icon" />
          </template>
    ...
    <template #append>
      ...
      <div class="sidebar-footer">
        <v-list class="nav-list" density="compact" nav>
          <v-list-item v-for="item in footerItems" :key="item.route" :active="isActive(item.route)" class="nav-item"
            rounded="lg" :to="item.route">
            <template #prepend>
        ...
        <div v-if="!rail" class="user-section">
          ...
          <v-btn class="logout-btn" icon="mdi-logout" size="small" variant="text" @click="$emit('logout')" />
        </div>
      </div>
    </template>
  </v-navigation-drawer>
